...
public class MutableImage {
    ...
    private Bitmap compressTransformedBitmap(Bitmap originalBitmap, int quality, Matrix m) throws OutOfMemoryError {
        Bitmap bitmap = toBitmap(toJpeg(originalBitmap, quality));

        Bitmap transformBitmap = Bitmap.createBitmap(
                bitmap,
                0,
                0,
                bitmap.getWidth(),
    ...
    public void mirrorImage() throws ImageMutationFailedException {
        ...
        try {
            ...
            try {
                bitmap = compressTransformedBitmap(this.currentRepresentation, 80, m);
            } catch (OutOfMemoryError o2) {
                try {
                    bitmap = compressTransformedBitmap(this.currentRepresentation, 75, m);
    ...
    private void rotate(int exifOrientation) throws ImageMutationFailedException {
        ...
        try {
            ...
            try {
                transformedBitmap = compressTransformedBitmap(this.currentRepresentation, 80, bitmapMatrix);
            } catch (OutOfMemoryError o2) {
                try {
                    transformedBitmap = compressTransformedBitmap(this.currentRepresentation, 75, bitmapMatrix);
                } catch (OutOfMemoryError o3) {
                    o3.printStackTrace();
                } catch (Exception e1) {
                    e1.printStackTrace();
    ...
    public int getWidth() {
        return (this.currentRepresentation != null)
                ? this.currentRepresentation.getWidth()
                : 0;
    }

    public int getHeight() {
        return (this.currentRepresentation != null)
                ? this.currentRepresentation.getHeight()
                : 0;
    }
